...
namespace vk_test {
    ...
    public:
        ...
        static void cmdSetViewportAndScissor(VkCommandBuffer cmd, const VkExtent2D& viewport_size, float min_depth = 0.0F, float max_depth = 1.0F) {
            const VkViewport viewport{ 0.0F, 0.0F, float(viewport_size.width), float(viewport_size.height), min_depth, max_depth };
            const VkRect2D   scissor{ { 0, 0 }, viewport_size };
        ...
        static void cmdBindShaders(VkCommandBuffer cmd, const BindableShaders& shaders, bool with_mesh = false) {
            std::array<VkShaderStageFlagBits, 7> shader_type = {
            ...
            vkCmdBindShadersEXT(cmd, with_mesh ? 7 : 5, shader_type.data(), &shaders.vertex);
        }
        ...
        std::vector<VkColorComponentFlags>   colorWriteMasks{ VK_COLOR_COMPONENT_R_BIT | VK_COLOR_COMPONENT_G_BIT | VK_COLOR_COMPONENT_B_BIT | VK_COLOR_COMPONENT_A_BIT };
        ...
        void cmdApplyDynamicStates(VkCommandBuffer cmd, std::span<const VkDynamicState> dynamic_states) const;

        // returns true if not handled
        bool cmdApplyDynamicState(VkCommandBuffer cmd, VkDynamicState dynamic_state) const;
    };
    ...
    public:
        ...
        VkResult createGraphicsPipeline(VkDevice device, VkPipelineCache cache, const GraphicsPipelineState& graphics_state, VkPipeline* p_pipeline);

    protected:
        void buildPipelineCreateInfo(VkGraphicsPipelineCreateInfo& create_info_temp, const GraphicsPipelineState& graphics_state);
    ...
    inline bool GraphicsPipelineState::cmdApplyDynamicState(VkCommandBuffer cmd, VkDynamicState dynamic_state) const {
