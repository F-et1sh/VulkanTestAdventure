...
void VKHppTest::DeviceManager::PickPhysicalDevice() {
    ...
    if (device_iterator != devices.end())
        ...
        RUNTIME_ERROR("ERROR : Failed to find a suitable GPU");
}

void VKHppTest::DeviceManager::CreateLogicalDevice() {
...
void VKHppTest::DeviceManager::CreateCommandPool() {
...
void VKHppTest::DeviceManager::CreateCommandBuffers() {
...
void VKHppTest::DeviceManager::configureDebugMessengerCreateInfo(vk::DebugUtilsMessengerCreateInfoEXT& create_info) const noexcept {
...
std::vector<const char*> VKHppTest::DeviceManager::getRequiredExtensions() const {
...
vk::Format VKHppTest::DeviceManager::findSupportedFormat(const std::vector<vk::Format>& candidates, vk::ImageTiling tiling, vk::FormatFeatureFlags features) const {
...
vk::Format VKHppTest::DeviceManager::findDepthFormat() const {
...
uint32_t VKHppTest::DeviceManager::findMemoryType(uint32_t type_filter, vk::MemoryPropertyFlags properties) const {
...
vk::raii::CommandBuffer VKHppTest::DeviceManager::beginSingleTimeCommands() const {
    vk::CommandBufferAllocateInfo alloc_info{
        m_CommandPool,                    // Command Pool
        vk::CommandBufferLevel::ePrimary, // Level
...
void VKHppTest::DeviceManager::endSingleTimeCommands(vk::raii::CommandBuffer& command_buffer) const {
    command_buffer.end();
...
uint32_t VKHppTest::DeviceManager::findQueueFamilies(vk::PhysicalDevice device, vk::QueueFlagBits flags) const {
    // find the index of the first queue family that supports graphics
